...
Inside the function block a local variable `age` is declared and initialized to the value of the passed in paramater plus 1, 
and then returned
...
On line 12 console.log is called and passed in the `addOneYearToAge` function which in turn is called with the `age` variable passed in as an argument; it returns and logs 26 to the console
On line 13 console.log is called again and passed in `age`, and once again it outputs 25 
...
The nesteded array calls the map method. Each subarray is passed into the callback. The parameter `arr` is assigned a copy of the reference of each subarray on each iteration. 'Arr` then calls the inner map method. The `num` parameter is assigned a copy of the number value. On each innovation of callback, the expression `num * 2` returns a new number to to the callback. On first innovaction of the outer map callback, the each innovation of the inner callback returns value , 2 and 4 to the inner map method which then map add to a new array. The outer map callback returns [2,4 ] to outer map which outer map append array to a new array. On the second innocation of the outer map callback, the inner map callback return the value of 6 and 8 to the inner map which inner map append to the new array. Outer map callback return the array [6,8] to the outer map, which outer map append to the new array and returns  a reference to a new array of  [[2,4], [6,8]].
